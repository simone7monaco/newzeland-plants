...
from utils import plot_curve, plot_sample
from utils import build_optimizer, objectview, get_known_mask, mask_edge

def train_gnn(data, args, log_path, device=torch.device('cpu')):
    ...
    if args.transfer_dir: # this ensures the valid mask is consistant
        load_path = f'./nzspecies/test/{args.transfer_dir}/'
    ...
    with tqdm.trange(args.epochs) as pbar:
        for epoch in pbar:
            ...
            with torch.no_grad():
                ...
                pbar.set_postfix(loss=train_loss, test_rmse=test_rmse, test_l1=test_l1)
                # if args.valid > 0.:
