...
def get_fixed_known_mask(edge_num, n_splits=5, split:int=0, seed:int=0):
    """
    set as unknown the remaining part with respect to the split in the kfold
    """
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=seed)
    kf_splits = list(kf.split(np.arange(edge_num)))
    known_mask = torch.zeros(edge_num, dtype=torch.bool)
    known_mask[kf_splits[split][0]] = True
    return known_mask
